...
import (
    "crawler/config"
    crawl "crawler/docker/crawl/client"
    "crawler/engine"
...
func main() {
    itemSaverServiceName := os.Getenv("ITEM_SAVER_SERVICE_NAME")
    if itemSaverServiceName == "" {
        log.Fatal("must be specity itemSaver service name in environment variable.")
    }

    crawlServiceName := os.Getenv("CRAWL_SERVICE_NAME")
    if crawlServiceName == "" {
        log.Fatal("must be specity crawl service name in environment variable.")
    }

    itemChan, err := itemsaver.ItemSaver(fmt.Sprintf("%s:%d", itemSaverServiceName, config.ItemServerRPCPort))
    if err != nil {
        log.Fatal(err)
    }
    ...
    processor := crawl.CreateProcessor(crawlServiceName)
    ...
    e := engine.ConcurrentEngine{
        ...
        RedisHost:        fmt.Sprintf("%s:%d", redisServiceName, 6379),
    }
